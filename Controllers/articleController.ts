...
export const getAllArticles = async (
  ...
  try {
    ...
    const articlesWithAuthor = await Promise.all(articles.map(async (article) => {
      ...
      if (article.id) {
        const user = await User.findById(article.id).select('firstName').lean();
        if (user) {
...
export const blockedArticle = async (
  ...
  try {
    ...
    const blockedArticles: IArticle[] = await Article.find({ blocks: userId })
...
export const unblockArticle = async (
  ...
  try {
    ...
    if (findArticle.blocks && findArticle.blocks.includes(userId)) {
      ...
      findArticle.blocks = findArticle.blocks.filter(id => id.toString() !== userId);
